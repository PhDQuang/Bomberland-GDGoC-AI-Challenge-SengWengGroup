...
class Agent:
    team_id = "AdvancedHeuristicBot_V4"
    ...
    def _decide(self, ctx, started):
        ...
        if current_bomb["legal"] and current_bomb["score"] >= self._bomb_threshold(current_bomb):
            ...
            return self.BOMB
        if time.perf_counter() - started > 0.096:
    ...
    def _choose_objective(self, ctx, started):
        ...
        if time.perf_counter() - started < 0.091:
        ...
        if time.perf_counter() - started < 0.093:
            future_bomb = self._best_future_bomb_move(ctx, reach, started)
            if future_bomb is not None and (best is None or future_bomb[0] > best[0]):
                best = future_bomb
    ...
    def _score_bomb_at(self, ctx, pos):
        ...
        if ctx["phase"] == "end":
            ...
            base_territory = self._evaluate_territory(ctx)
            new_territory = self._evaluate_territory(ctx, hypothetical_bomb=hypo_bomb_obj)
            if new_territory > base_territory + 3:
                endgame_trap_bonus += 4.0
    ...
    def _best_future_bomb_move(self, ctx, reach, started):
        if ctx["my_bombs_left"] <= 0:
            return None
            
        best = None
        import time
        
        # Sort targets by distance (dist is index 0 in the tuple (dist, action))
        targets = sorted(reach.items(), key=lambda item: item[1][0])
        
        for pos, (dist, action) in targets:
            # Anytime break condition
            if time.perf_counter() - started > 0.094:
                break
                
            if pos in ctx["bomb_positions"]:
                continue
                
            bomb_result = self._score_bomb_at(ctx, pos)
            
            if not bomb_result["legal"]:
                continue
                
            # Penalize the score by distance, distance means delay
            score = bomb_result["score"] - dist * 0.75
    ...
    def _fallback_action(self, ctx):
        ...
        for action in ctx["valid_actions"]:
            ...
            if npos in ctx["enemy_risk"]:
                score -= 3.0 if ctx["phase"] == "end" else 1.0
            if action == self.STOP:
                score -= 0.25
            
            center_x, center_y = ctx["grid"].shape[0] // 2, ctx["grid"].shape[1] // 2
            score -= (abs(npos[0] - center_x) + abs(npos[1] - center_y)) * 0.05
            
            territory = self._evaluate_territory(ctx, my_pos=npos)
            score += territory * 0.1
    ...
    def _evaluate_territory(self, ctx, my_pos=None, hypothetical_bomb=None):
        grid = ctx["grid"]
        bombs = list(ctx["bombs"])
        danger_at = dict(ctx["danger_at"])
        
        if hypothetical_bomb:
            bombs.append(hypothetical_bomb)
            blast = self._blast_tiles(grid, hypothetical_bomb["pos"][0], hypothetical_bomb["pos"][1], hypothetical_bomb["radius"])
            for tile in blast:
                danger_at.setdefault(tile, set()).add(7)

        start_my = my_pos if my_pos else ctx["my_pos"]
        
        from collections import deque
        q = deque()
        seen = {}
        
        q.append((start_my, 0, self.agent_id))
        seen[start_my] = (0, self.agent_id)
        
        for enemy in ctx["enemies"]:
            epos = enemy["pos"]
            q.append((epos, 0, enemy["id"]))
            seen[epos] = (0, enemy["id"])
            
        my_area = 0
        enemy_areas = {enemy["id"]: 0 for enemy in ctx["enemies"]}
        
        while q:
            pos, t, owner = q.popleft()
            ...
            for action in self.DIRS:
                ...
                if not self._passable(grid, npos[0], npos[1]):
                    continue
                if self._bomb_blocks(pos, npos, t + 1, bombs):
                    continue
                if self._is_deadly(npos, t + 1, danger_at):
                    continue
                    
                if npos not in seen:
                    seen[npos] = (t + 1, owner)
                    q.append((npos, t + 1, owner))
                    
        max_enemy_area = max(enemy_areas.values()) if enemy_areas else 0
