...
class Agent:
    ...
    def _bomb_threshold(self, bomb):
        ...
        if bomb["enemy_hits"] or bomb["trap_score"] >= 1.5:
            if phase == "early":
                return 6.8 + safety_tax
            if phase == "end":
                return 3.7 + safety_tax
            return (5.0 if self.turn < 120 else 4.2) + safety_tax
        if bomb["boxes"] >= 3:
            return (4.6 if phase == "early" else 5.4 if phase == "end" else 5.0) + safety_tax
        if bomb["boxes"] == 2:
            return (3.8 if phase == "early" else 5.2 if phase == "end" else 4.1) + safety_tax
        if bomb["boxes"] == 1:
            if phase == "end":
                return 5.8 + safety_tax
            return (3.0 if phase == "early" and self.turn < 180 else 3.8) + safety_tax
        return (4.5 if phase == "end" else 6.0) + safety_tax
    ...
    def _score_bomb_at(self, ctx, pos):
        ...
        if ctx["phase"] == "end":
            score += len(enemy_hits) * 2.2 + trap_score * 1.2
            if boxes > 0 and not enemy_hits and trap_score < 1.0:
                score -= 1.6
        elif ctx["phase"] == "early":
            if enemy_hits and trap_score < 1.5:
                score -= 1.4
            score += min(boxes, 3) * 0.35
        if slack < 3:
            score -= (3 - slack) * 1.6
        if escape_area <= 3:
            score -= 2.0
    ...
    def _item_value(self, ctx, cell):
        if cell == self.ITEM_CAPACITY:
            if ctx["my_bombs_left"] <= 1:
                return 5.0 if ctx["phase"] == "end" else 7.4
            return 2.6 if ctx["phase"] == "end" else 4.4
        if cell == self.ITEM_RADIUS:
            if ctx["my_bonus"] <= 1:
                return 4.8 if ctx["phase"] == "end" else 6.9
            if ctx["my_bonus"] < self.MAX_RADIUS - 1:
                return 2.3 if ctx["phase"] == "end" else 3.7
        return 0.0
