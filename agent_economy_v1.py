...
class Agent:
    ...
    def _bomb_threshold(self, bomb):
        ...
        safety_tax = 0.0
        if bomb.get("escape_area", 10) <= 5:
            safety_tax += 1.0
        if bomb["enemy_hits"] or bomb["trap_score"] >= 1.5:
            return (7.2 if phase == "early" else 4.8 if phase == "mid" else 4.0) + safety_tax
        if bomb["boxes"] >= 3:
            return (4.1 if phase == "early" else 5.0) + safety_tax
        if bomb["boxes"] == 2:
            return (3.4 if phase == "early" else 4.2) + safety_tax
        if bomb["boxes"] == 1:
            return (2.8 if phase == "early" and self.turn < 180 else 4.0) + safety_tax
        return 6.3 + safety_tax
    ...
    def _score_bomb_at(self, ctx, pos):
        ...
        if ctx["phase"] == "early":
            score += boxes * 0.7
            if enemy_hits and trap_score < 2.0:
                score -= 2.0
    ...
    def _item_value(self, ctx, cell):
        if cell == self.ITEM_CAPACITY:
            if ctx["my_bombs_left"] <= 1:
                return 8.2 if ctx["phase"] == "early" else 7.0
            return 5.0 if ctx["phase"] == "early" else 4.0
        if cell == self.ITEM_RADIUS:
            if ctx["my_bonus"] <= 1:
                return 7.6 if ctx["phase"] == "early" else 6.5
            if ctx["my_bonus"] < self.MAX_RADIUS - 1:
                return 4.3 if ctx["phase"] == "early" else 3.5
        return 0.0
