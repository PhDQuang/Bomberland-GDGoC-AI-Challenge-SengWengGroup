...
class Agent:
    ...
    def _observe_transition(self, obs):
        ...
        for row in obs.get("bombs", []):
            ...
            if timer <= 0:
                continue
            key = (bx, by, owner, timer)
            stable_key = (bx, by, owner)
            observed_bombs.add(stable_key)
            if owner == self.agent_id and stable_key not in self.seen_bombs:
        ...
        if self.prev_players is not None:
            prev_my = self.prev_players[self.agent_id]
            if int(my[4]) > int(prev_my[4]):
                self.telemetry["own_items"] += int(my[4]) - int(prev_my[4])
                self.telemetry["last_progress_turn"] = self.turn
            if int(my[3]) > prev_max_bombs_left:
                self.telemetry["own_items"] += int(my[3]) - prev_max_bombs_left
                self.telemetry["last_progress_turn"] = self.turn

        if self.prev_grid is None or self.prev_players is None:
            return

        exploded_self_blast = set()
        ...
        for bx, by, timer, owner in self.prev_bombs:
            ...
            if radius is None and 0 <= owner < len(self.prev_players):
                ...
                radius = 1
            exploded_self_blast |= self._blast_tiles(self.prev_grid, bx, by, radius)

        if exploded_self_blast:
            boxes = 0
            for x, y in exploded_self_blast:
                if int(self.prev_grid[x, y]) == self.BOX and int(grid[x, y]) != self.BOX:
                    boxes += 1
            if boxes:
                self.telemetry["own_boxes"] += boxes
                self.telemetry["last_progress_turn"] = self.turn

            for i, prev_p in enumerate(self.prev_players):
                if i == self.agent_id or int(prev_p[2]) != 1:
                    continue
                if int(players[i][2]) == 0 and (int(prev_p[0]), int(prev_p[1])) in exploded_self_blast:
                    self.telemetry["own_kills"] += 1
                    self.telemetry["enemy_deaths"] += 1
                    self.telemetry["last_progress_turn"] = self.turn
    ...
    def _best_box_position_move(self, ctx, reach):
        ...
        for x in range(1, grid.shape[0] - 1):
            for y in range(1, grid.shape[1] - 1):
                ...
                if ctx["tempo"] in ("economy", "unstuck"):
                    value += boxes * 0.9
                elif ctx["tempo"] in ("duel", "survive_duel"):
                    value *= 0.45
                elif ctx["tempo"] in ("pressure", "tiebreak"):
                    value *= 0.75
    ...
    def _best_chase_space_move(self, ctx, reach):
        ...
        for pos, path in reach.items():
            ...
            if pos in ctx["bomb_positions"] or pos in ctx["enemy_risk"]:
                continue
            nearest_enemy_dist = min(self._manhattan(pos, e["pos"]) for e in ctx["enemies"])
            if nearest_enemy_dist > 6 and ctx["tempo"] != "tiebreak":
                continue
